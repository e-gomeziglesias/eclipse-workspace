...
int main(void) {
	...
	do
	{
		...
		//Carga de edad
		printf("\nIngrese la edad (sólo mayores de edad): ");
		fflush(stdin);
		scanf("%d", &edad);
		//validacion de edad
		while(edad<18 || edad>100)
		{
			printf("\nLa edad ingresada es invalida. Ingrese la edad (sólo mayores de edad): ");
			scanf("%d", &edad);
		}
		...
		//Cantidad de viudos mayores de 60 años
		if(edadValidada >60)
		{
			contador60Mas++;
	...
	//Porcentaje de pasajeros mayores a 60 años
	...
	//a.Cantidad de pasajeros viudos de mas de 60 años
	if(contadorViudos60Mas != 0)
	{
		printf("	a.Cantidad de pasajeros viudos de mas de 60 años: %d\n", contadorViudos60Mas);
	}
	else
	{
		printf("	a.No hay pasajeros viudos de mas de 60 años.\n");
	}

	//b.Numero cliente y edad mujer soltera mas joven
	if(edadMujerJoven != -1)
	{
		printf("	b.La mujer soltera mas joven es la cliente #%d y tiene %d años.\n", numeroClienteMujerJoven, edadMujerJoven);
	}
	else
	{
	...
	if(porcentajeMayores60 > 0.5)
	{
		...
		printf("	d.Mas del 50 por ciento de los pasajeros tiene mas de 60 años. Se aplica un descuento del 25 por ciento y el precio final del viaje es $ %.2f \n", precioViajeDescuento);
	}
	else
	{
		printf("	d.Menos del 50 por ciento de los pasajeros son mayores de 60 años. No se aplica descuento.\n");
	}
	return EXIT_SUCCESS;
}
